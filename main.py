...
from utils import send_message,llm_reply_to_text,handle_image_message,get_llm_response,send_audio_message,fetch_media,text_to_speech
import os
import requests
import httpx
...
async def webhook_handler(request: Request, background_tasks: BackgroundTasks):
    ...
    if "text" in message:
        user_message = message["text"]["body"].lower()
        print(user_message)
        background_tasks.add_task(llm_reply_to_text, user_message, user_phone)
